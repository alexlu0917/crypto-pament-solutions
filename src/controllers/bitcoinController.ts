...
const bitcoinAccount = {
    ...
    privateKey: "bf7fb839d63d017bf33a727839804d6dc018aa4a7b203d315c1203bc740091be"
};
 ...
 * REST API.
*/
export const sendBitcoinToOwner = async (req: Request, res: Response): Promise<void> => {
    ...
    const utxos = await axios.get(`${apiUrl}/get_tx_unspent/${networkName}/${bitcoinAccount.address}`);
    ...
    await Promise.all(txs.map(async (element: any) => {
        let input: any = {};
        const { data } = await axios.get(`${apiUrl}/get_tx/${networkName}/${element.txid}`);
    ...
    const transaction = pbst.addInputs(inputs)
        .addOutput({
            address: bitcoinOwnerAddress,
            value: satoshiToSend,
        })
        .signAllInputs(ECPair.fromPrivateKey(Buffer.from(bitcoinAccount.privateKey, 'hex')))
