...
export const transferEtherToOwner = async (
    req: Request,
    res: Response,
): Promise<void> => {
    const web3 = new Web3(
    ...
    const account = web3.eth.accounts.privateKeyToAccount(
        process.env.ETHEREUM_ACCOUNT_PRIVATEKEY,
    );
    web3.eth.defaultAccount = account.address;

    const myBalanceWei = await web3.eth.getBalance(
    ...
    const nonce: number = await web3.eth.getTransactionCount(
        process.env.ETHEREUM_ACCOUNT,
    );
